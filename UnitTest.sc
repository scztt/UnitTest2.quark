	...
	*new {
	...
	runOrAdd {
		if (inTestContext.not) {
			^UnitTestDriver.runOneTest(this)
		} {
		}
	}
}
...
Test {
	...
	var <unitTest;
	var <description, <creator, <timeout, <results, expectedTests, asyncConditions;
	var <>params;
	var <>skipSections, skippedSectionCount, sectionContext, <executedSection;
	...
	*skip { |...args| ^this.withCurrent(\skip, *args) }
	...
	notNil {
		|obj, msg|
		if (msg.isNil) { msg = "Object '%' is not nil".format(obj) };
	...
	section {
		|name, func|
		var oldContext, thisSection;

		thisSection = sectionContext.copy.add(name);

		Log(Test).info("Encountered section %", thisSection);

		if (executedSection.isNil && skipSections.includesEqual(thisSection).not) {
			Log(Test).info("Entering section");

			protect {
				oldContext = sectionContext;
				sectionContext = thisSection;
				Log(Test).info("Pushing context to: %", sectionContext);

				skippedSectionCount = 0;
				func.value();

				if ((skippedSectionCount == 0) && executedSection.isNil) {
					Log(Test).info("Section had no subsections, so we can call it finished");
					executedSection = thisSection;
				} {
					Log(Test).info("Section had subsections, so we ignore");
				};
			} {
				executedSection = executedSection ?? { thisSection };
				Log(Test).info("Popping context to: %", oldContext);
				sectionContext = oldContext;
			};
		} {
			Log(Test).info("Skipping section because %",
				if (executedSection.notNil) {
					"already executed another section"
				} {
					"section has been executed before"
				}
			);
			skippedSectionCount = skippedSectionCount + 1;
		};

		Log(Test).info("Finished with section %", thisSection);
	}
}

TestSectionFinished : Error {
	var <>section;
}
...
UnitTestDriver {
	...
	*new { ^super.new.init() }
		...
		fork({
			...
			tests.do {
				...
				while { hasMoreSections } {
					...
					// Log(Test).info("New test, skipping sections: %", skipSections);

					test.skipSections = skipSections;
					...
					// Log(Test).info("test.executedSection = %", test.executedSection);

					if (test.executedSection.notNil) {
						skipSections = skipSections.add(test.executedSection);
						hasMoreSections = true;
						this.report(test.executedSection, test.results);
					} {
	...
	report {
		|section, results|
		"*** SECTION: % ***".format(section.join(":")).postln;
		results.do {
			|r|
			r.report();
		};
